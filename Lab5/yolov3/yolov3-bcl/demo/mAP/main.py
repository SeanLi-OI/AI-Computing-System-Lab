  ...
  for line in lines_list:
    try:
      if "difficult" in line:
          ...
          if (len(line_list) == 5):
            class_name, left, top, right, bottom = line_list
          elif (len(line_list) == 6):
            class_name = line_list[0] + ' ' + line_list[1]
            left, top, right, bottom = line.split()[2:]
    except ValueError:
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    for line in lines:
      try:
        line_list = line.split()
        #print ("predict lenth line =", len(line_list))
        if (len(line_list) == 6):
          tmp_class_name, confidence, left, top, right, bottom = line_list
        elif (len(line_list) == 7):
          tmp_class_name = line_list[0] + ' ' + line_list[1]
          confidence, left, top, right, bottom = line.split()[2:]
